<template>
  <div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
      <h2 class="text-xl font-semibold mb-4">{{ $t('pokemon.filter.title') }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          <label class="block text-sm font-medium mb-1">{{ $t('pokemon.filter.name') }}</label>
          <input 
        ...
        <div>
          <label class="block text-sm font-medium mb-1">{{ $t('pokemon.filter.id') }}</label>
          <input 
            v-model="filters.id" 
            type="number" 
              min="1"
        ...
        <div>
          <label class="block text-sm font-medium mb-1">{{ $t('pokemon.filter.type') }}</label>
          <input 
        ...
        <div>
          <label class="block text-sm font-medium mb-1">{{ $t('pokemon.filter.species') }}</label>
          <input 
    ...
    const displayedPokemon = computed(() => {
      ...
      return pokemonList.value.filter(pokemon => {
        ...
        if (filters.value.name) {
          matchesName = pokemon.name.toLowerCase().startsWith(filters.value.name.toLowerCase())
        }
        
        if (filters.value.id) {
          matchesId = pokemon.id.toString() === (filters.value.id)
        }
        
        if (filters.value.type) {
          matchesType = pokemon.types.some(typeInfo => 
            typeInfo.type.name.toLowerCase().startsWith(filters.value.type.toLowerCase())
          )
        }
        
        if (filters.value.species) {
          matchesSpecies = pokemon.species.name.toLowerCase().startsWith(filters.value.species.toLowerCase())
        }
