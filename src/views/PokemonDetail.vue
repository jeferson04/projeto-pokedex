<template>
  <div>
    ...
    <!-- Pokemon Detail -->
    <div v-else-if="pokemon" class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
      <div class="flex flex-col md:flex-row">
        ...
        <div class="md:w-1/3 mb-6 md:mb-0">
          ...
          <div class="mt-4 text-center">
            <h1 class="text-3xl font-bold capitalize mb-2">{{ pokemon.name || translatedName }}</h1>
          ...
          <div class="mt-6 grid grid-cols-2 gap-4">
            <div class="text-center">
              <div class="text-sm text-gray-500">{{ $t('pokemon.height') }}</div>
              <div class="font-semibold">{{ (pokemon.height / 10).toFixed(1) }}m</div>
            </div>
            <div class="text-center">
              <div class="text-sm text-gray-500">{{ $t('pokemon.weight') }}</div>
              <div class="font-semibold">{{ (pokemon.weight / 10).toFixed(1) }}kg</div>
            </div>
            <div class="text-center col-span-2">
              <div class="text-sm text-gray-500">{{ $t('pokemon.baseExperience') }}</div>
        ...
        <div class="md:w-2/3 md:pl-8">
          ...
          <div class="border-b border-gray-200 mb-6">
            <div class="flex flex-wrap -mb-px">
              <button 
                ...
                :class="activeTab === tab.value ? 'border-blue-500 text-blue-600' : 'border-transparent hover:border-gray-300'"
              >
                {{ tab.label }}
              </button>
            </div>
          ...
          <div v-if="activeTab === 'sprites'" class="space-y-6">
            <h2 class="text-xl font-semibold mb-4">{{ $t('pokemon.sprites') }}</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              <div v-for="(sprite, key) in allSprites" :key="key" class="text-center">
                <img :src="sprite" :alt="`${pokemon.name} ${key}`" class="mx-auto h-24 w-24 object-contain" />
                <div class="mt-2 text-xs text-gray-500 capitalize">{{ formatSpriteKey(key) }}</div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'moves'" class="space-y-6">
            <h2 class="text-xl font-semibold mb-4">{{ $t('pokemon.moves') }}</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              >
                <div class="font-medium capitalize">{{ formatMoveName(move.move.name) }}</div>
                <div class="text-sm text-gray-500">
                  Level: {{ getLearnLevel(move) }}
                </div>
              </div>
            </div>
            <div v-if="pokemon.moves.length > 30" class="text-center text-gray-500 mt-4">
          ...
          <div v-else-if="activeTab === 'evolutions'" class="space-y-6">
            <h2 class="text-xl font-semibold mb-4">{{ $t('pokemon.evolutions') }}</h2>
            <div v-if="evolutionChain && evolutionChain.length > 0" class="flex flex-wrap justify-center items-center gap-4">
              <template v-for="(evo, index) in evolutionChain" :key="index">
                ...
                <div v-if="index < evolutionChain.length - 1" class="text-2xl">→</div>
              </template>
            </div>
          ...
          <div v-else-if="activeTab === 'games'" class="space-y-6">
            <h2 class="text-xl font-semibold mb-4">{{ $t('pokemon.games') }}</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              <div 
              ...
              >
                <div class="font-medium capitalize">{{ formatGameName(gameIndex.version.name) }}</div>
                <div class="text-sm text-gray-500">
...
export default {
  ...
  setup(props) {
    ...
    const fetchPokemonData = async () => {
      try {
        ...
        if (species.value.evolution_chain) {
          const evolutionResponse = await api.getEvolutionChain(species.value.evolution_chain.url)
          const chain = evolutionResponse.data.chain
          
          // Process evolution chain
          const processedEvolutionChain = []
          ...
          const processChain = async (chain, evolutions = []) => {
            ...
            evolutions.push({
              ...
              image: pokemonDetail.data.sprites.other['official-artwork'].front_default || 
                    pokemonDetail.data.sprites.front_default
            })
    ...
    const allSprites = computed(() => {
      ...
      // Process other sprite collections
      if (pokemon.value.sprites.other) {
        Object.entries(pokemon.value.sprites.other).forEach(([collectionName, collection]) => {
