      ...
      >
        <div class="flex items-center mb-4 md:mb-0">
          ...
          <div class="ml-6 space-x-2">
            <button
              ...
              :class="{
                'bg-white text-blue-600': $i18n.locale === locale,
                'bg-blue-700': $i18n.locale !== locale,
              }"
            >
              {{ locale.toUpperCase() }}
            </button>
        ...
        <nav class="flex space-x-4">
          ...
          <template v-if="!isLoggedIn">
            <router-link to="/signup" class="hover:text-blue-200">{{
              $t("nav.signup")
            }}</router-link>
          </template>
          <button v-else @click="logout" class="hover:text-blue-200">
            {{ $t("nav.logout") }}
          </button>
        </nav>
      </div>
